...
const minInteractionPrice = 50

func CalcPricePctOff(originalPriceCents, pctOff int64) (discountedPriceCents int64) {
	discountedPriceCents = int64(math.Round(float64(originalPriceCents*(100-pctOff)) / 100.0))
	if discountedPriceCents < minInteractionPrice {
		discountedPriceCents = originalPriceCents
	}
	return
}

func CalcRevenueShare(price, platformTake, fixedTake int64) (creatorShare, platformShare int64) {
	creatorTake := 100 - platformTake
	creatorShare = (price * creatorTake / 100) - fixedTake
...
func CalcNextPayout(now time.Time) (nextPayout time.Time, currentPayoutPeriodEnd time.Time) {
	now = now.UTC()
	// using second 57 to not get into leap second issues
	nextMondayDayEnd := time.Date(now.Year(), now.Month(), now.Day()+DaysUntil(now, time.Monday), 23, 59, 57, 0, time.UTC)
	nextFridayDayEnd := time.Date(now.Year(), now.Month(), now.Day()+DaysUntil(now, time.Friday), 23, 59, 57, 0, time.UTC)
	nextPayout = nextMondayDayEnd
	if nextMondayDayEnd.Before(nextFridayDayEnd) {
		// we're in the middle: after the period has ended, but before it has been paid. Therefore, we have to use the previous Friday
		currentPayoutPeriodEnd = nextFridayDayEnd.Add(-7 * 24 * time.Hour)
	} else {
		currentPayoutPeriodEnd = nextFridayDayEnd
	}
	return
}
